            ...
            <div className="flex flex-col md:flex-row gap-6 max-w-6xl mx-auto p-4 bg-white">
                ...
                <div className="w-full md:w-1/2  bg-white ">
                    <div className='border-2 border-gray-500  p-4 rounded-xl '>
                        <h2 className="text-xl font-semibold text-[#1a1a85] mb-6">Billing Details</h2>
                        <form>
                            <div className="mb-4">
                    ...
                    <div className="border-2 md:invisible hidden border-gray-500 rounded-xl p-6 bg-white ">
                        ...
                        <button
                            ...
                            className={`w-full ${cart.length === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#00008B] hover:bg-[#000070]'} text-white py-3 px-4 rounded-full transition-colors text-lg font-semibold`}
                        >
                            Place order
                        </button>
                    </div>
                ...
                <div className="w-full md:w-1/2 flex flex-col gap-6">
                    ...
                    <div className="border-2 border-gray-500 rounded-xl p-6 bg-white">
                        <h2 className="text-xl font-semibold text-[#1a1a85] mb-3">Your Order</h2>
                        ...
                        {cart.length > 0 ? (
                            cart.map((item) => (
                                <div key={`${item.id}-${item.accessId}`} className="flex justify-between items-start  py-2">
                                    <div className="flex flex-col gap-1 text-sm text-black">
                                        <p className="font-medium">{item.title} × {item.quantity}</p>
                                    </div>
                                    <span className="text-sm font-medium text-black">₹{(item.price * item.quantity).toFixed(2)}</span>
                                </div>
                            ))
                        ) : (
